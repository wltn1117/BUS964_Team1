...
# 1. 데이터 불러오기
# =========================================================
df_2009 = pd.read_excel("./data/4_online_retail_II.xlsx", sheet_name="Year 2009-2010")
df_2010 = pd.read_excel("./data/4_online_retail_II.xlsx", sheet_name="Year 2010-2011")
...
rfm = df.groupby("CustomerID").agg({
    "InvoiceDate": lambda x: (snapshot_date.to_period("M") - x.max().to_period("M")).n,
    "InvoiceNo": "nunique",
    "Revenue": "sum"
}).reset_index()
...
# 7. 고객 × 상품 통합 테이블
# =========================================================
customer_product = df.groupby(["CustomerID", "StockCode", "Description"]).agg({
    "Quantity": "sum",
    "Revenue": "sum"
}).reset_index()
...
# 주문 단위 요약
order_summary = df.groupby(['CustomerID', 'InvoiceNo'], as_index=False).agg(
